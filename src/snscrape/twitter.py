# USING THE WHOLE OFFICIAL LIBRARY CAUSES PROBLEMS WITH PYINSTALLER
...
class Tweet(snscrape.base.Item):
    ...
    username = snscrape.base._DeprecatedProperty(
        "username", lambda self: self.user.username, "user.username"
    )
    outlinks = snscrape.base._DeprecatedProperty(
        "outlinks",
    ...
    outlinksss = snscrape.base._DeprecatedProperty(
        "outlinksss",
        lambda self: " ".join(x.url for x in self.links) if self.links else "",
        "links (url attribute)",
    )
    ...
    tcooutlinksss = snscrape.base._DeprecatedProperty(
        "tcooutlinksss",
        lambda self: " ".join(x.tcourl for x in self.links) if self.links else "",
        "links (tcourl attribute)",
    )
...
class UnifiedCard(Card):
    ...
    def __post_init__(self):
        if (self.components is None) == (self.swipeableLayoutSlides is None):
            raise ValueError(
                "did not get exactly one of components or swipeableLayoutSlides"
            )
        if self.components and not all(
            k in self.componentObjects for k in self.components
        ):
            raise ValueError("missing components")
        if self.swipeableLayoutSlides and not all(
            s.mediumComponentKey in self.componentObjects
            and s.componentKey in self.componentObjects
            for s in self.swipeableLayoutSlides
        ):
            raise ValueError("missing components")
        if any(
            c.destinationKey not in self.destinations
            for c in self.componentObjects.values()
            if hasattr(c, "destinationKey")
        ):
            raise ValueError("missing destinations")
        if any(
            b.destinationKey not in self.destinations
            for c in self.componentObjects.values()
            if isinstance(c, UnifiedCardButtonGroupComponentObject)
            for b in c.buttons
        ):
            raise ValueError("missing destinations")
        mediaKeys = []
        ...
        mediaKeys.extend(
            d.mediumKey for d in self.destinations.values() if d.mediumKey is not None
        )
        mediaKeys.extend(
            a.iconMediumKey
            for l in (self.apps.values() if self.apps is not None else [])
            for a in l
            if a.iconMediumKey is not None
        )
        if any(k not in self.media for k in mediaKeys):
            raise ValueError("missing media")
        if any(
            c.appKey not in self.apps
            for c in self.componentObjects.values()
            if hasattr(c, "appKey")
        ):
            raise ValueError("missing apps")
        if any(
            d.appKey not in self.apps
            for d in self.destinations.values()
            if d.appKey is not None
        ):
            raise ValueError("missing apps")
...
class User(snscrape.base.Entity):
    ...
    rawDescription: typing.Optional[
        str
    ] = None  # Raw description with the URL(s) intact
    renderedDescription: typing.Optional[
        str
    ] = None  # Description as it's displayed on the web interface with URLs replaced
...
class _CLIGuestTokenManager(GuestTokenManager):
    ...
    @token.setter
    def token(self, token):
        super(type(self), type(self)).token.__set__(
            self, token
...
class _TwitterAPIScraper(snscrape.base.Scraper):
    ...
    def _ensure_guest_token(self, url=None):
        if self._guestTokenManager.token is None:
            ...
            r = self._get(
                self._baseUrl if url is None else url,
            ...
            if not self._guestTokenManager.token:
                ...
                if not o.get("guest_token"):
                    raise snscrape.base.ScraperException(
                        "Unable to retrieve guest token"
                    )
    ...
    def _get_api_data(self, endpoint, apiType, params):
        ...
        if apiType is _TwitterAPIType.GRAPHQL:
            params = urllib.parse.urlencode(
                {"variables": json.dumps(params, separators=(",", ":"))},
        ...
        r = self._get(
            ...
            params=params,
            headers=self._apiHeaders,
            responseOkCallback=self._check_api_response,
        )
        try:
            obj = r.json()
        except json.JSONDecodeError as e:
            raise snscrape.base.ScraperException(
                "Received invalid JSON from Twitter"
            ) from e
        return obj
        ...
        while True:
            ...
            if apiType is _TwitterAPIType.V2:
                ...
                if "user" in obj["data"]:
                    ...
                    instructions = obj["data"]["user"]["result"]["timeline"][
                        "timeline"
                    ]["instructions"]
                else:
                    # TweetDetail
            ...
            for instruction in instructions:
                ...
                for entry in entries:
                    if not (
                        entry["entryId"].startswith("sq-cursor-")
                        or entry["entryId"].startswith("cursor-")
                    ):
                        continue
                    if apiType is _TwitterAPIType.V2:
                        entryCursor = entry["content"]["operation"]["cursor"]["value"]
                        entryCursorStop = entry["content"]["operation"]["cursor"].get(
                            "stopOnEmptyResponse", None
                        )
                    elif apiType is _TwitterAPIType.GRAPHQL:
                        cursorContent = entry["content"]
                        while (
                            cursorContent.get("itemType") == "TimelineTimelineItem"
                            or cursorContent.get("entryType") == "TimelineTimelineItem"
                        ):
                            cursorContent = cursorContent["itemContent"]
                        entryCursor, entryCursorStop = cursorContent[
                            "value"
                        ], cursorContent.get("stopOnEmptyResponse", None)
                    if entry["entryId"] == f"sq-cursor-{dir}" or entry[
                        "entryId"
                    ].startswith(f"cursor-{dir}-"):
                    ...
                    ):  # E.g. 'offensive' replies button
                        promptCursor = entryCursor
                    elif (
                        direction is _ScrollDirection.BOTH
                        and bottomCursorAndStop is None
                        and (
            ...
            if not newCursor or (stopOnEmptyResponse and tweetCount == 0):
                ...
                if promptCursor is not None:
                    ...
                    direction is _ScrollDirection.BOTH
                    and bottomCursorAndStop is not None
                ):
                    dir = "bottom"
    ...
    def _count_tweets(self, entries):
        return sum(
            ...
            or entry["entryId"].startswith("tweet-")
            for entry in entries
        )

    def _v2_timeline_instructions_to_tweets(
        ...
        for instruction in obj["timeline"]["instructions"]:
            ...
            for entry in entries:
                if entry["entryId"].startswith("sq-I-t-") or entry[
                    ...
                    yield from self._v2_instruction_tweet_entry_to_tweet(
                        entry["entryId"], entry["content"], obj
                    )
                elif (
                    includeConversationThreads
                    and entry["entryId"].startswith("conversationThread-")
                    and not entry["entryId"].endswith("-show_more_cursor")
                ):
                    for item in entry["content"]["timelineModule"]["items"]:
                        if item["entryId"].startswith("tweet-"):
                            yield from self._v2_instruction_tweet_entry_to_tweet(
                                item["entryId"], item, obj
                            )

    def _v2_instruction_tweet_entry_to_tweet(self, entryId, entry, obj):
        if "tweet" in entry["item"]["content"]:
            if (
                "promotedMetadata" in entry["item"]["content"]["tweet"]
            ):  # Promoted tweet aka ads
                return
            if (
            ...
            if (
                "tweet" not in entry["item"]["content"]["tombstone"]
            ):  # E.g. deleted reply
                return
            if (
                entry["item"]["content"]["tombstone"]["tweet"]["id"]
                not in obj["globalObjects"]["tweets"]
            ):
                _logger.warning(
    ...
    def _make_tweet(
        ...
        kwargs["renderedContent"] = self._render_text_with_urls(
            tweet["full_text"], tweet["entities"].get("urls")
        )
        kwargs["user"] = user
        ...
        kwargs["conversationId"] = (
            tweet["conversation_id"]
            if "conversation_id" in tweet
            else int(tweet["conversation_id_str"])
        )
        ...
        if inReplyToTweetId := tweet.get("in_reply_to_status_id_str"):
            ...
            if "inReplyToUser" not in kwargs:
                kwargs["inReplyToUser"] = User(
                    username=tweet["in_reply_to_screen_name"], id=inReplyToUserId
                )
        ...
        if tweet.get("place"):
            ...
            if (
                "coordinates" not in kwargs
                and tweet["place"].get("bounding_box")
        ...
        if card:
            ...
            if hasattr(card, "url") and "//t.co/" in card.url:
                ...
                for u in candidates:
                    ...
                    _logger.warning(
                        f"Could not translate t.co card URL on tweet {tweetId}"
                    )
        return Tweet(**kwargs)
    ...
    def _make_medium(self, medium, tweetId):
        if medium["type"] == "photo":
            if (
                "?format=" in medium["media_url_https"]
                or "&format=" in medium["media_url_https"]
            ):
                return Photo(
            ...
            if format not in ("jpg", "png"):
                _logger.warning(
                    f"Skipping photo with unknown format on tweet {tweetId}: {format!r}"
                )
                return
            return Photo(
            ...
            if medium["type"] == "video":
                ...
                if (
                    (ext := medium.get("ext"))
                    and (mediaStats := ext.get("mediaStats"))
                    and isinstance(r := mediaStats["r"], dict)
                    and "ok" in r
                    and isinstance(r["ok"], dict)
                ):
            ...
            _logger.warning(
                f'Unsupported medium type on tweet {tweetId}: {medium["type"]!r}'
            )
    ...
    def _make_card(self, card, apiType, tweetId):
        ...
        def _kwargs_from_map(keyKwargMap):
            ...
            return {
                kwarg: bindingValues[key]
                for key, kwarg in keyKwargMap.items()
                if key in bindingValues
            }
        ...
        if apiType is _TwitterAPIType.V2:
            ...
            for o in card["legacy"].get("user_refs", {}):
                ...
                if userId in userRefs:
                    _logger.warning(
                        f"Duplicate user {userId} in card on tweet {tweetId}"
                    )
                    continue
                if "legacy" in o:
        ...
        if apiType is _TwitterAPIType.V2:
            ...
            messyBindingValues = (
                (x["key"], x["value"]) for x in card["legacy"]["binding_values"]
            )
        for key, value in messyBindingValues:
            if "type" not in value:
                ...
                if key not in ("creator", "site"):
                    _logger.warning(
                        f"Skipping type-less card value {key!r} on tweet {tweetId}"
                    )
                continue
            if value["type"] == "STRING":
                bindingValues[key] = value["string_value"]
                if key.endswith("_datetime_utc"):
                    bindingValues[key] = datetime.datetime.strptime(
                        bindingValues[key], "%Y-%m-%dT%H:%M:%SZ"
                ...
                _logger.warning(
                    f'Unsupported card value type on {key!r} on tweet {tweetId}: {value["type"]!r}'
                )
        ...
        if cardName in (
            ...
            cardName.startswith(x)
            for x in ("poll2choice_", "poll3choice_", "poll4choice_")
        ) and cardName.split("_", 1)[1] in ("text_only", "image", "video"):
            ...
            if cardName.endswith("_image"):
                kwargs["medium"] = Photo(
                    previewUrl=bindingValues["image_small"],
                ...
                variants.append(
                    VideoVariant(
                        ...
                        url=bindingValues["player_hls_url"],
                        bitrate=None,
                    )
                )
                if "vmap" not in bindingValues["player_stream_url"]:
                    _logger.warning(
                        f"Non-VMAP URL in {cardName} player_stream_url on tweet {tweetId}"
                    )
                variants.append(
                    VideoVariant(
                        contentType="text/xml",
                        url=bindingValues["player_stream_url"],
                        bitrate=None,
                    )
                )
                kwargs["medium"] = Video(
                    thumbnailUrl=bindingValues["player_image_original"],
                    variants=variants,
            ...
            for l in ("one", "two", "three", "four"):
                if f"cta_{l}" in bindingValues:
                    kwargs["actions"].append(
                        PromoConvoAction(
                            label=bindingValues[f"cta_{l}"],
                            tweet=bindingValues[f"cta_{l}_tweet"],
                        )
                    )
            if "image" in cardName:
                kwargs["medium"] = Photo(
                    previewUrl=bindingValues["promo_image_small"],
                    fullUrl=bindingValues["promo_image_original"],
                )
                if "cover_promo_image" in bindingValues:
                    kwargs["cover"] = Photo(
                        previewUrl=bindingValues["cover_promo_image_small"],
                ...
                variants.append(
                    VideoVariant(
                        contentType=bindingValues["player_stream_content_type"],
                        url=bindingValues["player_stream_url"],
                        bitrate=None,
                    )
                )
                if bindingValues["player_stream_url"] != bindingValues["player_url"]:
                    if "vmap" not in bindingValues["player_url"]:
                        _logger.warning(
                            f"Non-VMAP URL in {cardName} player_url on tweet {tweetId}"
                        )
                    variants.append(
                        VideoVariant(
                            contentType="text/xml",
                            url=bindingValues["player_url"],
                            bitrate=None,
                        )
                    )
                kwargs["medium"] = Video(
            ...
            if cardName == "745291183405076480:broadcast":
                ...
                keyKwargMap = {
                    **keyKwargMap,
                    ...
                    "full_size_thumbnail_url": "thumbnailUrl",
                }
            kwargs = _kwargs_from_map(keyKwargMap)
            if "broadcaster_twitter_id" in bindingValues:
                kwargs["broadcaster"] = User(
                    id=int(bindingValues["broadcaster_twitter_id"]),
            ...
            kwargs["photo"] = Photo(
                previewUrl=bindingValues["event_thumbnail_small"],
                fullUrl=bindingValues.get("event_thumbnail_original")
                or bindingValues["event_thumbnail"],
            )
            ...
            kwargs = _kwargs_from_map(
                {
                    ...
                    "newsletter_image_original": "imageUrl",
                    "card_url": "url",
            ...
            return AmplifyCard(
                ...
                video=Video(
                    ...
                    variants=[
                        VideoVariant(
                            contentType=bindingValues["player_stream_content_type"],
                            url=bindingValues["amplify_url_vmap"],
                            bitrate=None,
                        )
                    ],
                ),
            ...
            if bindingValues["cta"] not in ctas:
                _logger.warning(
                    f'Unsupported message_me card cta on tweet {tweetId}: {bindingValues["cta"]!r}'
                )
                return
            return MessageMeCard(
                **_kwargs_from_map({"recipient": "recipient", "card_url": "url"}),
            ...
            if "type" in o:
                ...
                if unifiedCardType not in (
                    ...
                    _logger.warning(
                        f"Unsupported unified_card type on tweet {tweetId}: {unifiedCardType!r}"
                    )
                    return
                kwargs["type"] = unifiedCardType
            elif set(c["type"] for c in o["component_objects"].values()) not in (
                {"media", "twitter_list_details"},
                {"media", "community_details"},
            ):
            ...
            for k, v in o["component_objects"].items():
                if v["type"] == "details":
                    co = UnifiedCardDetailComponentObject(
                        content=v["data"]["title"]["content"],
                        destinationKey=v["data"]["destination"],
                    )
                elif v["type"] == "media":
                    co = UnifiedCardMediumComponentObject(
                        mediumKey=v["data"]["id"],
                        destinationKey=v["data"]["destination"],
                    )
                    ...
                    if not all(b["type"] == "cta" for b in v["data"]["buttons"]):
                        _logger.warning(
                            f"Unsupported unified_card button_group button type on tweet {tweetId}"
                        )
                        return
                    buttons = [
                        UnifiedCardButton(
                            text=b["action"][0].upper()
                            + re.sub("[A-Z]", lambda x: f" {x[0]}", b["action"][1:]),
                            destinationKey=b["destination"],
                        )
                        for b in v["data"]["buttons"]
                    ]
                    co = UnifiedCardButtonGroupComponentObject(buttons=buttons)
                elif v["type"] == "swipeable_media":
                    media = [
                        UnifiedCardSwipeableMediaMedium(
                            mediumKey=m["id"], destinationKey=m["destination"]
                        )
                        for m in v["data"]["media_list"]
                    ]
                    co = UnifiedCardSwipeableMediaComponentObject(media=media)
                elif v["type"] == "app_store_details":
                    co = UnifiedCardAppStoreComponentObject(
                        appKey=v["data"]["app_id"],
                        destinationKey=v["data"]["destination"],
                    )
                    ...
                    co = UnifiedCardTwitterCommunityDetailsComponentObject(
                        ...
                        destinationKey=v["data"]["destination"],
                        membersFacepile=[
                            self._user_to_user(u)
                            for u in map(o["users"].get, v["data"]["members_facepile"])
                            if u
                        ],
                    )
                else:
                    _logger.warning(
                        f'Unsupported unified_card component type on tweet {tweetId}: {v["type"]!r}'
                    )
                    return
            ...
            if "app_store_data" in o:
                ...
                for k, v in o["app_store_data"].items():
                    ...
                    for var in v:
                        ...
                        vKwargs = {
                            kwarg: var[key]
                            for key, kwarg in vKwargsMap.items()
                            if key in var
                        }
                        ...
                        vKwargs["url"] = (
                            f'https://play.google.com/store/apps/details?id={var["id"]}'
                            if var["type"] == "android_app"
                            else f'https://itunes.apple.com/app/id{var["id"]}'
                        )
                        if (
                            "iconMediumKey" in vKwargs
                            and vKwargs["iconMediumKey"] not in kwargs["media"]
                        ):
            ...
            if "layout" in o:
                if o["layout"]["type"] != "swipeable":
                    _logger.warning(
                        f'Unsupported unified_card layout type on tweet {tweetId}: {o["layout"]["type"]!r}'
                    )
                    return
                kwargs["swipeableLayoutSlides"] = [
                    UnifiedCardSwipeableLayoutSlide(
                        mediumComponentKey=v[0], componentKey=v[1]
                    )
                    for v in o["layout"]["data"]["slides"]
                ]
    ...
    def _tweet_to_tweet(self, tweet, obj):
        ...
        if (
            "quoted_status_id_str" in tweet
            and tweet["quoted_status_id_str"] in obj["globalObjects"]["tweets"]
        ):
            kwargs["quotedTweet"] = self._tweet_to_tweet(
                obj["globalObjects"]["tweets"][tweet["quoted_status_id_str"]], obj
            )
        if "card" in tweet:
            kwargs["card"] = self._make_card(
                tweet["card"], _TwitterAPIType.V2, self._get_tweet_id(tweet)
            )
    ...
    def _graphql_timeline_tweet_item_result_to_tweet(self, result):
        if result["__typename"] == "Tweet":
            ...
            raise snscrape.base.ScraperException(
                f'Unknown result type {result["__typename"]!r}'
            )
        tweet = result["legacy"]
        userId = int(result["core"]["user_results"]["result"]["rest_id"])
        user = self._user_to_user(
            result["core"]["user_results"]["result"]["legacy"], id_=userId
        )
        kwargs = {}
        if "retweeted_status_result" in tweet:
            kwargs[
                "retweetedTweet"
            ] = self._graphql_timeline_tweet_item_result_to_tweet(
        ...
        if "quoted_status_result" in result:
            ...
            if result["quotedRefResult"]["result"]["__typename"] == "TweetTombstone":
                ...
                kwargs["quotedTweet"] = TweetRef(
                    id=int(result["quotedRefResult"]["result"]["rest_id"])
                )
        elif "quoted_status_id_str" in tweet:
            kwargs["quotedTweet"] = TweetRef(id=int(tweet["quoted_status_id_str"]))
        if "card" in result:
            kwargs["card"] = self._make_card(
                result["card"], _TwitterAPIType.GRAPHQL, self._get_tweet_id(tweet)
            )
        return self._make_tweet(tweet, user, **kwargs)
    ...
    def _render_text_with_urls(self, text, urls):
        ...
        urlsSorted = sorted(
            urls, key=lambda x: x["indices"][0]
        )  # Ensure that they're in left to right appearance order
        assert all(
            url["indices"][1] <= nextUrl["indices"][0]
            for url, nextUrl in zip(urls, urls[1:])
        ), "broken URL indices"
    ...
    def _user_to_user(self, user, id_=None):
        ...
        kwargs["id"] = (
            id_ if id_ else user["id"] if "id" in user else int(user["id_str"])
        )
        kwargs["displayname"] = user["name"]
        kwargs["rawDescription"] = user["description"]
        kwargs["renderedDescription"] = self._render_text_with_urls(
            user["description"], user["entities"]["description"].get("urls")
        )
        if user["entities"]["description"].get("urls"):
            kwargs["descriptionLinks"] = [
                TextLink(
        ...
        if user.get("url"):
            ...
            kwargs["link"] = TextLink(
                text=entity.get("display_url"),
                url=entity.get("expanded_url", user["url"]),
                tcourl=user["url"],
                indices=tuple(entity["indices"]),
            )
...
class TwitterSearchScraper(_TwitterAPIScraper):
    ...
    def __init__(self, query, *, cursor=None, top=False, **kwargs):
        ...
        self._query = (
            query  # Note: may get replaced by subclasses when using user ID resolution
        )
        self._cursor = cursor
        self._top = top
    ...
    def _cli_setup_parser(cls, subparser):
        ...
        subparser.add_argument(
            "query",
            type=snscrape.base.nonempty_string("query"),
            help="A Twitter search string",
        )
...
class TwitterUserScraper(TwitterSearchScraper):
    ...
    def __init__(self, user, **kwargs):
        ...
        self._baseUrl = (
            ...
            else f"https://twitter.com/i/user/{self._user}"
        )
    ...
    def _get_entity(self):
        ...
        if not self._isUserId:
            ...
            fieldName = "userId"
            endpoint = (
                "https://twitter.com/i/api/graphql/I5nvpI91ljifos1Y3Lltyg/UserByRestId"
            )
        variables = {
        ...
        if (
            not obj["data"]
            or obj["data"]["user"]["result"]["__typename"] == "UserUnavailable"
        ):
            return None
        user = obj["data"]["user"]["result"]
        rawDescription = user["legacy"]["description"]
        renderedDescription = self._render_text_with_urls(
            rawDescription, user["legacy"]["entities"]["description"]["urls"]
        )
        link = None
        if user["legacy"].get("url"):
            ...
            link = TextLink(
                text=entity.get("display_url"),
                url=entity.get("expanded_url", user["legacy"]["url"]),
                tcourl=user["legacy"]["url"],
                indices=tuple(entity["indices"]),
            )
        label = None
    ...
    def is_valid_username(s):
        return (
            1 <= len(s) <= 15
            and s.strip(string.ascii_letters + string.digits + "_") == ""
        )
    ...
    def _cli_from_args(cls, args):
        return cls._cli_construct(
            args, user=int(args.user) if args.isUserId else args.user
        )
...
class TwitterProfileScraper(TwitterUserScraper):
    ...
    def get_items(self):
        ...
        for obj in self._iter_api_data(
            ...
            instructions = obj["data"]["user"]["result"]["timeline"]["timeline"][
                "instructions"
            ]
            if not gotPinned:
                for instruction in instructions:
                    if instruction["type"] == "TimelinePinEntry":
                        gotPinned = True
                        yield self._graphql_timeline_tweet_item_result_to_tweet(
                            instruction["entry"]["content"]["itemContent"][
                                "tweet_results"
                            ]["result"]
                        )
...
class TwitterHashtagScraper(TwitterSearchScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            "hashtag",
            type=snscrape.base.nonempty_string("hashtag"),
...
class TwitterTweetScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        if self._mode is TwitterTweetScraperMode.SINGLE:
            ...
            if not obj["data"]:
                return
            for instruction in obj["data"]["threaded_conversation_with_injections"][
                "instructions"
            ]:
                if instruction["type"] != "TimelineAddEntries":
                    continue
                for entry in instruction["entries"]:
                    if (
                        entry["entryId"] == f"tweet-{self._tweetId}"
                        and entry["content"]["entryType"] == "TimelineTimelineItem"
                        and entry["content"]["itemContent"]["itemType"]
                        == "TimelineTweet"
                    ):
                        yield self._graphql_timeline_tweet_item_result_to_tweet(
            ...
            for obj in self._iter_api_data(
                ...
                yield from self._graphql_timeline_instructions_to_tweets(
                    obj["data"]["threaded_conversation_with_injections"][
                        "instructions"
                    ],
                ...
                for obj in self._iter_api_data(
                    ...
                    for tweet in self._graphql_timeline_instructions_to_tweets(
                        ...
                        if tweet.id not in seenTweets:
                            ...
                            if (
                                tweet.id != self._tweetId
                            ):  # Already queued at the beginning
...
class TwitterListPostsScraper(TwitterSearchScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            "list",
            type=snscrape.base.nonempty_string("list"),
...
class TwitterTrendsScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        obj = self._get_api_data(
            "https://twitter.com/i/api/2/guide.json", _TwitterAPIType.V2, params
        )
        for instruction in obj["timeline"]["instructions"]:
