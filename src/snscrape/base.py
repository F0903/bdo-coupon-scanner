...
# https://raw.githubusercontent.com/JustAnotherArchivist/snscrape/master/snscrape/base.py
import abc
import copy
import dataclasses
import datetime
...
class _DeprecatedProperty:
    ...
    def __get__(self, obj, objType):
        if (
            obj is None
        ):  # if the access is through the class using _DeprecatedProperty rather than an instance of the class:
            return self
        warnings.warn(
...
def _json_dataclass_to_dict(obj):
    if isinstance(obj, _JSONDataclass) or dataclasses.is_dataclass(obj):
        ...
        return {
            _json_dataclass_to_dict(k): _json_dataclass_to_dict(v)
